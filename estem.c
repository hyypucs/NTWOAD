...
#if defined (_NOTUSE_ENGSTEM)
	return TRUE;
#else
            /* Part 1: Check to ensure the word is all alphabetic */
   for ( end = word; *end != EOS; end++ )
      if ( !isalpha((int)*end) ) return( FALSE );
      else *end = tolower( *end );
   end--;

    if (strlen(word) <= 3) return FALSE; // 3자 이하는 stemming이 무의미 
...
PUBLIC tBOOL EWordStem( tHCHAR *hstr )
{
	char word[40];
	tHCHAR *p = hstr;
	tINT nCount = 0;
...
#else
	while (*p) {
                if (!IsEnglish(*p))  return TRUE; // 영어일때만 한다.
                p++;
		nCount ++;
        }
	if (nCount >= 40) return TRUE; // except이것보다 크면 stemming하지 말자..

	hstr2str(hstr, (unsigned char *)word);
	if (Stem(word) == TRUE) {
		str2hstr((unsigned char *)word, hstr);
	}
	return TRUE;
#endif
}
