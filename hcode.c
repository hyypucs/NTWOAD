...
SpecialChar ksSpHdr=    {   0xa1, 12 - 1};
SpecialChar ksSpBody=   {   0xa1, 0xfe - 0xa1};
SpecialChar kssmSpHdr=  {   0xd9, 6 - 1};
...
#define MAX_HCODE 2350
unsigned short int KSTable[MAX_HCODE] = {   /*KS완성형문자를 조합형으로 나타낸 것임 */
...
unsigned Single[SINGLENUM] = {   /* 낱자 조합형코드 51자 */
...
int	DetectHcode(unsigned char *szStr)
{
   ...
   int	ks_count = 0;               
   long	i;               
   int     len = (int)strlen((char *)szStr);
   
   for ( i = 0 ; i < len ; i ++ ) {
	key = szStr[i];
      if(!(key & 0x80))                 /* 한글코드가 아니라면 */
         continue;

      first = key;                  /* 첫번째 코드값을 저장 */ 
      i++;
      second= szStr[i];
      ...
      if((hangul_code > 0xa1a1 && hangul_code < 0xa1ff) ||
        ...
        /* KS5061의 한자코드인가? */
         ...
         /* 처음으로 완성형코드를 만나면 */
         if(!ks_count)       
            code = KS_CODE;  /* 완성형코드로 세팅함 */
         if(ks_count++ >= KS_CHECK_COUNT) 
            break;               /* 일정한 횟수 이상 완성형코드면 더이상 비교를
                                    중지하고 루프를 탈출 */
      }
      else {
         code = KSSM_CODE;      /* 완성형이 아니면 조합형코드*/
         break;
      }
   }
   return code;                  /* 코드리턴 */
...
int CheckCode(const unsigned short int *item1, const unsigned short int *item2)
{
...
{
	int ch, c1, c2, flag= 0;
	unsigned short int johap;
	unsigned short int kscode;
	int j, len = (int)strlen((char *)szKSSMStr), k = 0;
	...
	for ( j =  0 ; j < len ; j ++ ) {
		...
		if (ch & 0x80) {
			...
			j ++;
			c2= szKSSMStr[j];
			johap= (((unsigned short int)c1<<8) + c2);

			if(c1>=kssmSpHdr.start && c1<=kssmSpHdr.start+kssmSpHdr.width) {           //특수문자
				  for(i= 0; i< SINGLENUM; i++)
					  if(johap == Single[i]) break;
				  ...
				  if (i < SINGLENUM) {
					...
					body=-1;
					if(c2>=kssmSpBody[0].start && c2 <=kssmSpBody[0].start+kssmSpBody[0].width)
						body=0;
					else if(c2 >=kssmSpBody[1].start &&c2<=kssmSpBody[1].start+kssmSpBody[1].width)
						body=1;
					if(body>-1) {
						i = c1 - kssmSpHdr.start;
						w=c2 - kssmSpBody[body].start;

						c1=ksSpHdr.start+i*2+body;
						c2=ksSpBody.start+w;
						flag=1;
					}
				}
			}
			else if(c1>=kssmHanjaHdr.start &&c1<=kssmHanjaHdr.start+kssmHanjaHdr.width) {		//한자
				if( !(c2 < 0x30 || ( c2 > 0x7E && c2 < 0x91 )) ) {
                                        if( c2 >= 0xA1 ) {
                                                c1 = (c1 - 0xE0) * 2 + 0xCA + 1;
                                        }
                                        else {
			...
			if (!flag) {
				p = (unsigned short int *)bsearch(&johap, KSTable, MAX_HCODE, sizeof(unsigned short int), (compar)CheckCode);
				if (p) {
					i = p - KSTable;
					c1= (i /94 + 0xb0);
					c2= (i%94) + 0xa1;
...
{
	int ch, c1, c2, index;
	unsigned short int johap, kscode;
	int i, len = (int)strlen((char *)szKSStr), k = 0;
	int flag = 0;
	int j, w;
	unsigned short int kssmcode;
	int bCvtOk = 1;
	...
	for ( j = 0 ; j < len ; j ++ ) {
		...
		if(ch & 0x80) {
			...
			j ++;
			c2= szKSStr[j];
			kscode= (((unsigned short int)c1<<8) + c2);


			if(c1>=ksSpHdr.start &&c1<=ksSpHdr.start+ksSpHdr.width &&c2 >=ksSpBody.start && c2<=ksSpBody.start+ksSpBody.width) {	//특수문자
				if (kscode >= 0xa4a1 && kscode < 0xa4a1 + SINGLENUM) {
					for ( i = 0 ; i < SINGLENUM ; i++ ) {
						if (i + 0xa4a1 == kscode) break;
					}
					if ( i < SINGLENUM) {
						kssmcode= Single[i];
						c1= kssmcode >>8;
					...
					c1=kssmSpHdr.start+i/2;
					c2=kssmSpBody[i%2].start+w;
					flag=1;
				}
			}
			else if(c1>=ksHanjaHdr.start &&c1<=ksHanjaHdr.start+ksHanjaHdr.width &&c2>=ksSpBody.start &&c2<=ksSpBody.start+ksSpBody.width) { //한자
				if( !(c2 < 0xA1 || c2 > 0xFE) ){
...
/*
				i=c1-ksHanjaHdr.start;
				w=c2-ksSpBody.start;

				c1=kssmHanjaHdr.start+i/2;
				c2=kssmSpBody[i%2].start+w;
				flag=1;
*/
			}
			...
			if (!flag) {
				...
				if (index >= 0 && index < MAX_HCODE) {
					johap= KSTable[index];
					c1= (int)(johap >>8);
					c2= (int)(johap &0x00ff);
				}
				else if (kscode >= 0xa4a1 && kscode < 0xa4a1 + SINGLENUM) {
					for ( i = 0 ; i < SINGLENUM ; i++ ) {
						if (i + 0xa4a1 == kscode) break;
					}
					if ( i < SINGLENUM) {
...
int KSSMHStr2KSHStr(tHCHAR *uszKSSMStr, tHCHAR *uszKSStr) 
{
	unsigned short int ch, c1, c2;
	int flag= 0;
	unsigned short int johap;
	unsigned short int kscode;
	int j, len = HStrlen(uszKSSMStr), k = 0;
	unsigned short int *p;
	int body, i, w;
	int bCvtOk = 1;
	...
	for ( j =  0 ; j < len ; j ++ ) {
		...
		if (ch & 0x8000) { // 한글이면 
			...
			uszKSStr[k++] = (unsigned short int)c1<<8 | (int)c2;
		  }
		else uszKSStr[k++] = ch;
	 } 
         uszKSStr[k] = 0x00;
...
int KSHStr2KSSMHStr(tHCHAR *uszKSStr, tHCHAR *uszKSSMStr)
{
	unsigned short int ch, c1, c2;
	int  index;
	unsigned short int johap, kscode;
	int i, len = HStrlen(uszKSStr), k = 0;
	int flag = 0;
	int j, w;
	unsigned short int kssmcode;
	int bCvtOk = 1;
...
*/
			...
			uszKSSMStr[k++] = (unsigned short int)c1<<8 | (int)c2;
		}
		else uszKSSMStr[k++] = ch;
	}
	uszKSSMStr[k] = '\0';
...
{
	unsigned char *p, *q;
	int len = HStrlen(uszKSSMStr);
	int nRetVal = 0;

	p = (unsigned char *)malloc(len * 2 + 1);
	if (p == NULL) return 0;
	q = (unsigned char *)malloc(len * 2 + 1);
	if (q == NULL) {
		pthread_cleanup_pop(0);
		free(p);
		return 0;
	}
	hstr2str(uszKSSMStr, p);
	nRetVal = KSSMStr2KSStr(p, q);
	if (nRetVal) {
		str2hstr(q, uszKSStr);
	}
	free(p);
...
int KSHStr2KSSMHStr(unsigned short int *uszKSStr, unsigned short int *uszKSSMStr)
{
	...
	hstr2str(uszKSStr, p);
	nRetVal = KSStr2KSSMStr(p, q);
	if (nRetVal) {
		str2hstr(q, uszKSSMStr);
	}
	free(p);
	free(q);
	return (nRetVal);
...
tBOOL checkChCode(tBYTE *szCh, tBOOL bKSSM, tINT nCheckCode)
{
	...
	c1 = szCh[0];
	c2 = szCh[1];
	code= (((unsigned short int)c1<<8) + c2);

	if (bKSSM) {
		if(c1>=kssmSpHdr.start && c1<=kssmSpHdr.start+kssmSpHdr.width) {  //특수문자
			if (nCheckCode == SPECIAL_CODE) return TRUE;
		}
		else if(c1>=kssmHanjaHdr.start &&c1<=kssmHanjaHdr.start+kssmHanjaHdr.width) {//한자
			if (nCheckCode == HANJA_CODE) return TRUE;
		}
		else {
			p = (unsigned short int *)bsearch(&code, KSTable, MAX_HCODE, sizeof(unsigned short int), (compar)CheckCode);
			if (p) {
				if (nCheckCode == HANGUL_CODE) return TRUE;
			}
		}
...
main( int argc, char *argv[])
{
	...
	char outBuf[1024];
	char outBuf1[1024];
	unsigned short int hinBuf[1024];
	unsigned short int houtBuf1[1024];
	unsigned short int houtBuf[1024];
	...
	str2hstr(inBuf, hinBuf);

	KSHStr2KSSMHStr(hinBuf, houtBuf);
	hstr2str(hinBuf, inBuf);
	hstr2str(houtBuf, outBuf);
	printf("%s -> %s\n", inBuf, outBuf);

	KSSMHStr2KSHStr(houtBuf, houtBuf1);
	hstr2str(houtBuf, outBuf);
	hstr2str(houtBuf1, outBuf1);
	printf("%s -> %s\n", outBuf, outBuf1);
	strcpy(inBuf, "한글");
	KSStr2KSSMStr(inBuf, outBuf);
	printf("%s -> %s\n", inBuf, outBuf);
	KSSMStr2KSStr(outBuf, outBuf1);
	printf("%s -> %s\n", outBuf, outBuf1);
	return 1;
}
#endif
